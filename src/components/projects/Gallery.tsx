...
const Gallery: React.FC<GalleryProps> = ({ images }) => {
    // Calculate layout: last image should always be full width
    // Alternate between full width and two-column layout
    const getImageLayout = (index: number, total: number) => {
        // Last image is always full
        if (index === total - 1) {
            return 'full';
        }

        // Calculate remaining images after current
        const remaining = total - index;

        // If only 2 images remain and we're not at the last, show as half
        if (remaining === 2) {
            return 'half';
        }

        // Alternate: start with full, then pair, then full, etc.
        // Count how many "full" images we've had before this position
        let fullCount = 0;
        let halfPairCount = 0;

        for (let i = 0; i < index; i++) {
            if (i === total - 1) break; // Skip last image in count
    ...
    const renderImages = () => {
        ...
        while (i < images.length) {
            ...
            if (layout === 'full') {
                ...
                // Check if there's a next image for the pair
                const hasNext = i + 1 < images.length && getImageLayout(i + 1, images.length) === 'half';
                ...
                if (hasNext) {
                    ...
                    // Fallback: render as full if no pair
                    elements.push(
                        <motion.div
                            key={i}
                            initial={{ opacity: 0, y: 30 }}
