...
const Navbar: React.FC = () => {
    ...
    return (
        <>
                ...
                <div className="max-w-7xl mx-auto px-6 lg:px-8">
                    <div className="flex flex-row justify-between items-center h-16">
                        <div className="flex items-center gap-10">
                            <Link
                                to="/"
                                className={`font-medium text-lg hover:opacity-80 transition-all relative z-50 ${isPanelOpen ? "text-black" : "text-white"
                                    }`}
                            >
                                Rishi Viswanathan
                            </Link>
                            <span className={`text-sm md:text-md relative z-50 ${isPanelOpen ? "text-black" : "text-white"
                                }`}>
                                {currentTime}
                            </span>
                        </div>
                        ...
                        <div className="flex items-center gap-10">
                            ...
                            <button
                                ...
                                className={`lg:hidden flex flex-col gap-1.5 w-8 h-8 justify-center items-center transition-colors ${isPanelOpen ? "text-black" : "text-white"
                                    }`}
                                aria-label="Menu"
                            >
            ...
            <div
                className={`fixed top-0 right-0 h-full w-full bg-white z-40 transition-transform duration-500 ease-in-out ${isPanelOpen ? "translate-x-0" : "translate-x-full"
                    }`}
            >
